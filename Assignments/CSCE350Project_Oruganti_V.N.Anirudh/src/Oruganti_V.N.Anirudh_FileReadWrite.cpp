...
#include <CSCE350Project_Oruganti_V.N.Anirudh/inc/Oruganti_V.N.Anirudh_FileReadWrite.h>
...
void FileReadWrite::writeFile(
    std::vector<std::vector<float>> executionTime,
  ...
  std::string averageExecutionTimePath =
      "Oruganti_V.N.Anirudh_averageExecutionTime.txt";

  dataOutputFile.open(executionTimePath.c_str());
  dataOutputFile << "Input Size\t"
                 << "Execution Time\n";
  for (std::vector<float>::size_type i = 0; i < executionTime.size() - 1; ++i) {
    for (std::vector<float>::size_type ii = 0; ii < 100; ++ii) {
      dataOutputFile << std::fixed << std::setprecision(10) << argv[i + 1]
                     << "\t" << executionTime[i][ii] << "\n";
    }
  }
  ...
  for (std::vector<float>::size_type i = 0; i < averageExecutionTime.size() - 1;
       ++i) {
    dataOutputFile << std::fixed << std::setprecision(10) << argv[i + 1] << "\t"
                   << averageExecutionTime[i][i] << "\n";
  }
  dataOutputFile.close();
}
